...
async function generatePackingListPDF(chain: {
  ...
  // ── Colors ──
  const BLUE_HEADER = [68, 114, 196] as [number,number,number];
  const BLUE_ROW    = [189, 215, 238] as [number,number,number];
  const YELLOW_ROW  = [255, 255, 153] as [number,number,number];
  const TOTAL_BG    = [220, 230, 241] as [number,number,number];
  ...
  doc.text("PACKING LIST / LISTE DE COLISAGE", margin, y + 7);
  y += 16;

  // ── Expéditeur / Destinataire ──
  const colMid = margin + colW / 2;
  doc.setFontSize(9);
  doc.setFont("helvetica", "bold");
  doc.text("EXPÉDITEUR:", margin + 20, y, { align: "center" });
  doc.text("DESTINATAIRE:", colMid + 20, y, { align: "center" });
  y += 5;

  doc.setFont("helvetica", "normal");
  const senderLines = [chain.senderName || "WALA France"];
  const recipLines  = [chain.recipientName || "—", chain.recipientAddress || ""].filter(Boolean);

  const maxLines = Math.max(senderLines.length, recipLines.length);
  for (let i = 0; i < maxLines; i++) {
    doc.text(senderLines[i] || "", margin + 20, y, { align: "center" });
    doc.text(recipLines[i]  || "", colMid + 20,  y, { align: "center" });
    y += 4.5;
  }

  y += 4;
  doc.setFont("helvetica", "bold");
  doc.text("Date:", margin + 20, y, { align: "center" });
  doc.setFont("helvetica", "normal");
  doc.text(new Date().toLocaleDateString("fr-FR"), colMid - 20, y);
  if (chain.orderRef) {
    doc.setFont("helvetica", "bold");
    doc.text("N° CDE:", colMid + 5, y);
    doc.setFont("helvetica", "normal");
    doc.text(chain.orderRef, colMid + 25, y);
  }
  y += 10;

  // ── Table header ──
  const cols = [
    { label: "N°",           x: margin,      w: 16,  align: "center" as const },
    { label: "Code Article", x: margin+16,   w: 32,  align: "center" as const },
    { label: "Description",  x: margin+48,   w: 74,  align: "center" as const },
    { label: "Lot",          x: margin+122,  w: 38,  align: "center" as const },
    { label: "Quantité",     x: margin+160,  w: 35,  align: "center" as const },
  ];
  const rowH = 7;
  ...
  doc.setFontSize(9);
  cols.forEach(c => {
    doc.text(c.label, c.x + c.w / 2, y + 4.8, { align: "center" });
  });
  y += rowH;

  // ── Rows — flatten all palettes ──
  doc.setFont("helvetica", "normal");
  doc.setFontSize(9);

  // Collect all valid lines with palette number
  ...
  // Alternate yellow/blue per palette number
  allRows.forEach((row, ri) => {
    const bg = row.pNum % 2 === 1 ? YELLOW_ROW : BLUE_ROW;
    doc.setFillColor(...bg);
    doc.rect(margin, y, colW, rowH, "F");
    doc.setTextColor(...BLACK);
    ...
    const cy = y + 4.8;
    doc.text("P" + row.pNum,        cols[0].x + cols[0].w/2, cy, { align: "center" });
    doc.text(row.ref,               cols[1].x + cols[1].w/2, cy, { align: "center" });

    // Description: truncate if too long
    const desc = row.ref.length > 30 ? row.ref : row.ref;
    const descText = doc.splitTextToSize(row.ref, cols[2].w - 2);
    doc.text(descText[0] || "", cols[2].x + 1, cy);

    doc.text(row.lot || "",         cols[3].x + cols[3].w/2, cy, { align: "center" });
    doc.text(row.qty > 0 ? String(row.qty) : "", cols[4].x + cols[4].w/2, cy, { align: "center" });
  ...
  doc.setFont("helvetica", "bold");
  doc.setTextColor(...BLACK);
  doc.text("TOTAL", margin + colW * 0.55, y + 4.8, { align: "center" });
  doc.text(String(totalQty), cols[4].x + cols[4].w / 2, y + 4.8, { align: "center" });
  y += rowH + 8;
  ...
  statsRows.forEach(([label, val]) => {
    doc.setFont("helvetica", "bold");
    doc.text(label, margin + 40, y, { align: "right" });
    doc.setFont("helvetica", "normal");
